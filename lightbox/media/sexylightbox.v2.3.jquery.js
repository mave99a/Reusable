...
(function($) {
  ...
  SexyLightbox = {
    ...
    overlay: {
      ...
      inject: function() {
        ...
        // Me cago en John Resig
        {
          ...
          this.shim = $('<iframe id="IF_'+new Date().getTime()+'" scrolling="no" frameborder=0 src="javascript:false;document.write(\'\');"></div>');
          this.shim.css({
            zIndex    : zIndex,
            position  : 'absolute',
            top       : 0,
      ...
      resize: function(x, y) {
        var win = { x: $(window).width(), y: $(document).height() };
    ...
    backwardcompatibility: function(option) {
      this.options.dir = option.dir || option.imagesdir || option.path || option.folder;
    ...
    initialize: function(options) {
      ...
      // fix the bug that background image can't be configured. // by mao
      this.options.imagesdir = this.options.dir;
    ...
    */
    replaceBox: function(data) {
      ...
      this.MoveBox = this.Wrapper.animate({
        ...
        'top' : (scroll.y + (size.y - (data.height + (this.MostrarNav ? 80 : 48))) / 2)
      }, {
    ...
    show: function(caption, url, rel, force) {
      ...
      if ( force=='image' || baseURL.match(imageURL) )
      {
          ...
          this.img.onload = $.bind(this, function(){
              ...
              this.lightbox.height = params['height'] - ((this.lightbox.title) ? 21 : 35);
      ...
      if (this.lightbox.title) this.MostrarNav = true;
